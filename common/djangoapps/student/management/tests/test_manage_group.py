...
class TestManageGroupCommand(TestCase):
    ...
    def test_invalid_input(self, initial_group_permissions, command_args, exception_message):
        ...
        assert exception_message in str(exc_context.exception).lower()
    ...
    def test_invalid_permission(self, initial_group_permissions):
        ...
        assert 'invalid permission option' in str(exc_context.exception).lower()
        self.check_group_permissions(initial_group_permissions)
        ...
        assert 'no installed app' in str(exc_context.exception).lower()
        assert 'nonexistent-label' in str(exc_context.exception).lower()
        self.check_group_permissions(initial_group_permissions)

        # invalid model name
        with pytest.raises(CommandError) as exc_context:
            call_command('manage_group', TEST_GROUP, '--permissions', 'auth:nonexistent-model:dummy-perm')
        assert 'nonexistent-model' in str(exc_context.exception).lower()
        self.check_group_permissions(initial_group_permissions)

        # invalid model name
        with pytest.raises(CommandError) as exc_context:
            call_command('manage_group', TEST_GROUP, '--permissions', 'auth:Group:nonexistent-perm')
        assert 'invalid permission codename' in str(exc_context.exception).lower()
        assert 'nonexistent-perm' in str(exc_context.exception).lower()
        self.check_group_permissions(initial_group_permissions)
