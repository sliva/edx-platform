...
class LoginFailuresAdminTest(TestCase):
    ...
    def test_unicode_username(self):
        ...
        """
        assert str(LoginFailures.objects.get(user=self.user)) == '§: 10 - {}'.format(self.user_lockout_until.isoformat())
...
class AllowedAuthUserFormTest(SiteMixin, TestCase):
    ...
    def test_form_with_invalid_site_configuration(self):
        ...
        assert error == "Please add a key/value 'THIRD_PARTY_AUTH_ONLY_DOMAIN/{site_email_domain}' in SiteConfiguration model's site_values field."
    ...
    def test_form_with_invalid_domain_name(self):
        ...
        assert error == "Email doesn't have {email_domain_name} domain name.".format(email_domain_name=self.email_domain_name)
