...
class RefundableTest(SharedModuleStoreTestCase):
    ...
    def test_refundable_when_certificate_exists(self, cutoff_date):
        ...
        assert not self.enrollment.refundable(user_already_has_certs_for=GeneratedCertificate.course_ids_with_certs_for_user(self.user))

        # Assert that can_refund overrides this and allows refund
        self.enrollment.can_refund = True
        assert self.enrollment.refundable()
        assert self.enrollment.refundable(user_already_has_certs_for=GeneratedCertificate.course_ids_with_certs_for_user(self.user))
