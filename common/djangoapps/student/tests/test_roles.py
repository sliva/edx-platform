...
class RolesTestCase(TestCase):
    ...
    def test_course_role(self):
        ...
        """
        assert not CourseStaffRole(self.course_key).has_user(self.student), 'Student has premature access to {}'.format(self.course_key)
        CourseStaffRole(self.course_key).add_users(self.student)
        assert CourseStaffRole(self.course_key).has_user(self.student), "Student doesn't have access to {}".format(six.text_type(self.course_key))

        # remove access and confirm
        CourseStaffRole(self.course_key).remove_users(self.student)
        assert not CourseStaffRole(self.course_key).has_user(self.student), 'Student still has access to {}'.format(self.course_key)

    def test_org_role(self):
        """
        Test that giving a user an org role enables access appropriately
        """
        assert not OrgStaffRole(self.course_key.org).has_user(self.student), 'Student has premature access to {}'.format(self.course_key.org)
        OrgStaffRole(self.course_key.org).add_users(self.student)
        assert OrgStaffRole(self.course_key.org).has_user(self.student), "Student doesn't have access to {}".format(six.text_type(self.course_key.org))

        # remove access and confirm
        OrgStaffRole(self.course_key.org).remove_users(self.student)
        if hasattr(self.student, '_roles'):
            del self.student._roles
        assert not OrgStaffRole(self.course_key.org).has_user(self.student), 'Student still has access to {}'.format(self.course_key.org)
    ...
    def test_org_and_course_roles(self):
        ...
        assert OrgInstructorRole(self.course_key.org).has_user(self.student), "Student doesn't have access to {}".format(six.text_type(self.course_key.org))
        assert CourseInstructorRole(self.course_key).has_user(self.student), "Student doesn't have access to {}".format(six.text_type(self.course_key))

        # remove access and confirm
        OrgInstructorRole(self.course_key.org).remove_users(self.student)
        assert not OrgInstructorRole(self.course_key.org).has_user(self.student), 'Student still has access to {}'.format(self.course_key.org)
        assert CourseInstructorRole(self.course_key).has_user(self.student), "Student doesn't have access to {}".format(six.text_type(self.course_key))

        # ok now keep org role and get rid of course one
        OrgInstructorRole(self.course_key.org).add_users(self.student)
        CourseInstructorRole(self.course_key).remove_users(self.student)
        assert OrgInstructorRole(self.course_key.org).has_user(self.student), 'Student lost has access to {}'.format(self.course_key.org)
        assert not CourseInstructorRole(self.course_key).has_user(self.student), "Student doesn't have access to {}".format(six.text_type(self.course_key))
