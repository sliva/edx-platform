...
class StubYouTubeServiceTest(unittest.TestCase):  # lint-amnesty, pylint: disable=missing-class-docstring
    ...
    def test_transcript_url_equal(self):
        ...
        assert ''.join(['<?xml version="1.0" encoding="utf-8" ?>', '<transcript><text start="1.0" dur="1.0">', 'Equal transcripts</text></transcript>']).encode('utf-8') == response.content
    ...
    def test_transcript_url_not_equal(self):
        ...
        assert ''.join(['<?xml version="1.0" encoding="utf-8" ?>', '<transcript><text start="1.1" dur="5.5">', 'Transcripts sample, different that on server', '</text></transcript>']).encode('utf-8') == response.content
