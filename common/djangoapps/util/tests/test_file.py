...
class FilenameGeneratorTestCase(TestCase):
    ...
    def test_filename_generator(self, course_key):
        ...
        assert u'foo_bar_baz_base_name_ø_1974-06-22-010203' == course_and_time_based_filename_generator(course_key, ' base` name ø ')
...
class StoreUploadedFileTestCase(TestCase):
    ...
    def verify_exception(self, expected_message, error):
        ...
        """
        assert expected_message == text_type(error.exception)
...
class TestUniversalNewlineIterator(TestCase):
    ...
    def test_only_one_line_no_trailing_newline(self, buffer_size):
        assert [thing.decode('utf-8') for thing in UniversalNewlineIterator(StringIO(u'foo'), buffer_size=buffer_size)] == ['foo']

    @ddt.data(1, 2, 999)
    def test_empty_file(self, buffer_size):
        assert [thing.decode('utf-8') for thing in UniversalNewlineIterator(StringIO(u''), buffer_size=buffer_size)] == []

    @ddt.data(1, 2, 999)
    def test_unicode_data(self, buffer_size):
        assert [(thing.decode('utf-8') if six.PY3 else thing) for thing in UniversalNewlineIterator(StringIO(u'héllø wo®ld'), buffer_size=buffer_size)] == [u'héllø wo®ld']
