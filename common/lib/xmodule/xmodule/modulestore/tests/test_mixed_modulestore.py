...
class TestMixedModuleStore(CommonMixedModuleStoreSetup):
    ...
    def test_get_modulestore_cache(self, default_ms):
        ...
        with check_exact_number_of_calls(self.store.default_modulestore, 'has_course', 1):
            assert self.store.default_modulestore == self.store._get_modulestore_for_courselike(course_key)
    ...
    def test_has_item(self, default_ms, max_find, max_send):
        ...
        with check_mongo_calls(max_find.pop(0), max_send):
            assert self.store.has_item(self.problem_x1a_1)
    ...
    def test_get_item(self, default_ms, max_find, max_send):
        ...
        with check_mongo_calls(max_find.pop(0), max_send):
            assert self.store.get_item(self.problem_x1a_1) is not None
            # lint-amnesty, pylint: disable=no-member
    ...
    def test_get_items_include_orphans(self, default_ms, expected_items_in_tree, orphan_in_items):
        ...
        assert set(['course', 'about']).issubset(set([item.location.block_type for item in items]))
        # lint-amnesty, pylint: disable=consider-using-set-comprehension
        # Assert that about is a detached category found in get_items
        assert [item.location.block_type for item in items if (item.location.block_type == 'about')][0] in DETACHED_XBLOCK_TYPES
        assert len(items) == 2
        ...
        assert set(['course', 'about']).issubset(set([item.location.block_type for item in items_in_tree]))
        # lint-amnesty, pylint: disable=consider-using-set-comprehension
    ...
    def test_get_parent_locations(self, default_ms, max_find, max_send):
        ...
        with check_mongo_calls(max_find, max_send):
            ...
            assert parent == self.vertical_x1a
            # lint-amnesty, pylint: disable=no-member
    ...
    def test_move_1_moved_1_unchanged(self, store_type):
        ...
        assert problem_item2.parent == self.vertical_x1a
        # lint-amnesty, pylint: disable=no-member
    ...
    def test_revert_to_published_root_draft(self, default_ms):
        ...
        self.store.publish(self.course.location, self.user_id)
        assert not self._has_changes(self.vertical_x1a)
        # lint-amnesty, pylint: disable=no-member
        ...
        assert self._has_changes(self.vertical_x1a)
        # lint-amnesty, pylint: disable=no-member
    ...
    def test_branch_setting(self, default_ms):
        ...
        def assertNumProblems(display_name, expected_number):
            ...
            """
            assert len(self.store.get_items(course_key.for_branch(None), settings={'display_name': display_name})) == expected_number
    ...
    def test_aside_crud(self, default_store):
        ...
        with MongoContentstoreBuilder().build() as contentstore:
            ...
            with self.store.default_store(default_store):
                ...
                assert not chapter_aside.fields['data_field'].is_set_on(chapter_aside), "data_field says it's assigned to {}".format(chapter_aside.data_field)
                assert not chapter_aside.fields['content'].is_set_on(chapter_aside), "content says it's assigned to {}".format(chapter_aside.content)
