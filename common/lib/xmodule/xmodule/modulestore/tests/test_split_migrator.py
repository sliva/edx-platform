    ...
    def compare_dags(self, presplit, presplit_dag_root, split_dag_root, published):  # lint-amnesty, pylint: disable=missing-function-docstring
        ...
        for name, field in six.iteritems(presplit_dag_root.fields):
            ...
            if should_check_field:
                assert getattr(presplit_dag_root, name) == getattr(split_dag_root, name), u'{}/{}: {} != {}'.format(split_dag_root.location, name, getattr(presplit_dag_root, name), getattr(split_dag_root, name))

        # compare children
        if presplit_dag_root.has_children:
            assert len(presplit_dag_root.get_children()) == len(split_dag_root.children), u"{0.category} '{0.display_name}': children  {1} != {2}".format(presplit_dag_root, presplit_dag_root.children, split_dag_root.children)
