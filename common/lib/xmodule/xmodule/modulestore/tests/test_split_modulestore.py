...
class SplitModuleItemTests(SplitModuleTest):
    ...
    def test_has_item(self, _from_json):
        ...
        assert not modulestore().has_item(BlockUsageLocator(locator.course_key.for_branch(BRANCH_NAME_PUBLISHED), block_type=locator.block_type, block_id=locator.block_id)), 'found in published head'
    ...
    def test_matching(self):
        ...
        assert modulestore()._block_matches({'field': ['distract', 'help', 'notme'], 'irrelevant': 2}, {'field': 'help'})
        ...
        assert modulestore()._block_matches({'a': 1, 'group_access': {'1': [1]}}, {'a': 1, 'group_access': {'$exists': True}})
        assert not modulestore()._block_matches({'a': 1, 'group_access': {'1': [1]}}, {'a': 111, 'group_access': {'$exists': True}})
...
class TestPublish(SplitModuleTest):
    ...
    def _check_course(self, source_course_loc, dest_course_loc, expected_blocks, unexpected_blocks):
        ...
        for expected in expected_blocks:
            ...
            assert source.update_version == pub_copy.source_version, u"Versions don't match for {}: {} != {}".format(expected, source.update_version, pub_copy.update_version)
            assert self.user_id == pub_copy.edited_by, '{} edited_by {} not {}'.format(pub_copy.location, pub_copy.edited_by, self.user_id)
...
class TestSchema(SplitModuleTest):
    ...
    def test_schema(self):
        ...
        for collection in [db_connection.course_index, db_connection.structures, db_connection.definitions]:
            assert collection.count_documents({'schema_version': {'$exists': False}}) == 0, '{0.name} has records without schema_version'.format(collection)
            assert collection.count_documents({'schema_version': {'$ne': SplitMongoModuleStore.SCHEMA_VERSION}}) == 0, '{0.name} has records with wrong schema_version'.format(collection)
