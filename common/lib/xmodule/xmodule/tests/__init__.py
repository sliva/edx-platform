...
class CourseComparisonTest(TestCase):
    ...
    def assertReferenceRelativelyEqual(self, reference_field, expected_block, actual_block):
        ...
        assert expected == actual, LazyFormat("Field {} doesn't match between usages {} and {}: {!r} != {!r}", reference_field.name, expected_block.scope_ids.usage_id, actual_block.scope_ids.usage_id, expected, actual)
    ...
    def assertFieldEqual(self, field, expected_block, actual_block):
        ...
        if isinstance(field, (Reference, ReferenceList, ReferenceValueDict)):
            ...
            assert expected == actual, LazyFormat("Field {} doesn't match between usages {} and {}: {!r} != {!r}", field.name, expected_block.scope_ids.usage_id, actual_block.scope_ids.usage_id, expected, actual)
