...
class ProctoringProviderTestCase(unittest.TestCase):
    ...
    def test_from_json_with_invalid_provider(self):
        ...
        assert context_manager.exception.args[0] == ['The selected proctoring provider, {}, is not a valid provider. Please select from one of {}.'.format(provider, allowed_proctoring_providers)]
