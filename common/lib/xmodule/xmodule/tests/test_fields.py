...
class DateTest(unittest.TestCase):  # lint-amnesty, pylint: disable=missing-class-docstring
    ...
    def test_enforce_type(self):
        ...
        assert DateTest.date.enforce_type('2012-12-31T23:00:01') == datetime.datetime(2012, 12, 31, 23, 0, 1, tzinfo=UTC)
        assert DateTest.date.enforce_type(1234567890000) == datetime.datetime(2009, 2, 13, 23, 31, 30, tzinfo=UTC)
        assert DateTest.date.enforce_type(datetime.datetime(2014, 5, 9, 21, 1, 27, tzinfo=UTC)) == datetime.datetime(2014, 5, 9, 21, 1, 27, tzinfo=UTC)
    ...
    def test_to_json(self):
        ...
        """
        assert DateTest.date.to_json(datetime.datetime.strptime('2012-12-31T23:59:59Z', '%Y-%m-%dT%H:%M:%SZ')) == '2012-12-31T23:59:59Z'
        assert DateTest.date.to_json(DateTest.date.from_json('2012-12-31T23:59:59Z')) == '2012-12-31T23:59:59Z'
        assert DateTest.date.to_json(DateTest.date.from_json('2012-12-31T23:00:01-01:00')) == '2012-12-31T23:00:01-01:00'
        with pytest.raises(TypeError):
...
class TimedeltaTest(unittest.TestCase):  # lint-amnesty, pylint: disable=missing-class-docstring
    ...
    def test_from_json(self):
        assert TimedeltaTest.delta.from_json('1 day 12 hours 59 minutes 59 seconds') == datetime.timedelta(days=1, hours=12, minutes=59, seconds=59)
    ...
    def test_enforce_type(self):
        ...
        assert TimedeltaTest.delta.enforce_type(datetime.timedelta(days=1, seconds=46799)) == datetime.timedelta(days=1, seconds=46799)
    ...
    def test_to_json(self):
        assert '1 days 46799 seconds' == TimedeltaTest.delta.to_json(datetime.timedelta(days=1, hours=12, minutes=59, seconds=59))
...
class RelativeTimeTest(unittest.TestCase):  # lint-amnesty, pylint: disable=missing-class-docstring
    ...
    def test_enforce_type(self):
        ...
        assert RelativeTimeTest.delta.enforce_type(datetime.timedelta(days=1, seconds=46799)) == datetime.timedelta(days=1, seconds=46799)
