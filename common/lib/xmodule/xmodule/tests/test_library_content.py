...
class TestLibraryContentAnalytics(LibraryContentTest):
    ...
    def _assert_event_was_published(self, event_type):
        ...
        assert len(self.publisher.call_args[0]), 3
    ...
    def test_assigned_event(self):
        ...
        assert event_data == {'location': six.text_type(self.lc_block.location), 'added': [block_info], 'result': [block_info], 'previous_count': 0, 'max_count': 1}
    ...
    def test_removed_invalid(self):
        ...
        event_data = self._assert_event_was_published("removed")
        assert event_data['removed'] == [{'usage_key': six.text_type(deleted_block_key), 'original_usage_key': None, 'original_usage_version': None, 'descendants': []}]
        assert event_data['result'] == [{'usage_key': six.text_type(keep_block_key), 'original_usage_key': six.text_type(keep_block_lib_usage_key), 'original_usage_version': six.text_type(keep_block_lib_version), 'descendants': []}]
