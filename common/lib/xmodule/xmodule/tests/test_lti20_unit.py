...
class LTI20RESTResultServiceTest(LogicTest):
    ...
    def test_lti20_put_like_delete_success(self):
        ...
        assert called_grade_obj == {'user_id': self.USER_STANDIN.id, 'value': None, 'max_value': None, 'score_deleted': True}
    ...
    def test_lti20_put_set_score_success(self):
        ...
        (_, evt_type, called_grade_obj), _ = self.system.publish.call_args  # lint-amnesty, pylint: disable=unpacking-non-sequence
        assert evt_type == 'grade'
        assert called_grade_obj == {'user_id': self.USER_STANDIN.id, 'value': 0.1, 'max_value': 1.0, 'score_deleted': False}
    ...
    def test_lti20_get_with_score_success(self):
        ...
        # Now assert
        assert response.status_code == 200
        assert response.json == {'@context': 'http://purl.imsglobal.org/ctx/lis/v2/Result', '@type': 'Result', 'resultScore': SCORE, 'comment': COMMENT}
