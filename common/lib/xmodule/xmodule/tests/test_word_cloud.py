...
class WordCloudBlockTest(TestCase):
    ...
    def test_indexibility(self):
        ...
        module_system = get_test_system()
        block = WordCloudBlock(module_system, DictFieldData(self.raw_field_data), Mock())
        assert block.index_dictionary() == {'content_type': 'Word Cloud', 'content': {'display_name': 'Word Cloud Block', 'instructions': 'Enter some random words that comes to your mind'}}
