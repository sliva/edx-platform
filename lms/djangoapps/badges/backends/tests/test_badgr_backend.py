...
class BadgrBackendTestCase(ModuleStoreTestCase, EventTrackingTestCase):
    ...
    def test_urls(self):
        ...
        # lint-amnesty, pylint: disable=no-member
        assert self.handler._badge_url('test_slug_here') == 'https://example.com/v1/issuer/issuers/test-issuer/badges/test_slug_here'
        assert self.handler._assertion_url('another_test_slug') == 'https://example.com/v1/issuer/issuers/test-issuer/badges/another_test_slug/assertions'
    ...
    def test_create_badge(self, post):
        ...
        assert kwargs['data'] == {'name': 'Test Badge', 'slug': EXAMPLE_SLUG, 'criteria': 'https://example.com/syllabus', 'description': "Yay! It's a test badge."}
