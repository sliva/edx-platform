...
class TestEmailSendFromDashboardMockedHtmlToText(EmailSendFromDashboardTestCase):
    ...
    def test_send_to_self(self):
        ...
        assert mail.outbox[0].from_email == u'"{course_display_name}" Course Staff <{course_name}-no-reply@example.com>'.format(course_display_name=self.course.display_name, course_name=self.course.id.course)
    ...
    def test_chunked_queries_send_numerous_emails(self, email_mock):
        ...
        assert mock_factory.emails_sent == ((((1 + len(self.staff)) + len(self.students)) + LARGE_NUM_EMAILS) - len(optouts))
...
class TestCourseEmailContext(SharedModuleStoreTestCase):
    ...
    def verify_email_context(self, email_context, scheme):
        ...
        assert email_context['course_url'] == '{}://edx.org/courses/{}/{}/{}/'.format(scheme, self.course_org, self.course_number, self.course_run)
        assert email_context['course_image_url'] == '{}://edx.org/c4x/{}/{}/asset/images_course_image.jpg'.format(scheme, self.course_org, self.course_number)
