...
class CertificatesDetailRestApiTest(AuthAndScopesTestMixin, SharedModuleStoreTestCase, APITestCase):
    ...
    def assert_success_response_for_student(self, response):
        """ This method is required by AuthAndScopesTestMixin. """
        assert response.data == {'username': self.student.username, 'status': CertificateStatuses.downloadable, 'is_passing': True, 'grade': '0.88', 'download_url': 'www.google.com', 'certificate_type': CourseMode.VERIFIED, 'course_id': six.text_type(self.course.id), 'created_date': self.now}
