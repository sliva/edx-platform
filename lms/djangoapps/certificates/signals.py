    ...
    for enrollment in user_enrollments:
        if is_using_certificate_allowlist_and_is_on_allowlist(user, enrollment.course_id):
            log.info(
                ...
                'made to generate an allowlist certificate. Id verification status is {status}'.format(
                    course=enrollment.course_id, user=user.id, status=expected_verification_status))
