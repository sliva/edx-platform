...
class BlockParentsMapTestCase(TransformerRegistryTestMixin, ModuleStoreTestCase):
    ...
    def _check_results(self, user, expected_accessible_blocks, blocks_with_differing_access, transformers):
        ...
        for i, xblock_key in enumerate(self.xblock_keys):
            ...
            assert block_structure_result == (i in expected_accessible_blocks), u'block_structure return value {0} not equal to expected value for block {1} for user {2}'.format(block_structure_result, i, user.username)
            ...
            if blocks_with_differing_access:
                ...
                if i in blocks_with_differing_access:
                    assert block_structure_result != has_access_result, u'block structure ({0}) & has_access ({1}) results are equal for block {2} for user {3}'.format(block_structure_result, has_access_result, i, user.username)
                else:
                    assert block_structure_result == has_access_result, u'block structure ({0}) & has_access ({1}) results not equal for block {2} for user {3}'.format(block_structure_result, has_access_result, i, user.username)
