...
class CoursesTest(ModuleStoreTestCase):
    ...
    def test_get_course_func_with_access_error(self, course_access_func_name):
        ...
        assert text_type(error.exception) == 'Course not found.'
        assert error.exception.access_response.error_code == 'not_visible_to_user'
        assert not error.exception.access_response.has_access
    ...
    def test_get_courses_with_filter(self):
        ...
        for filter_, expected_courses in test_cases:
            assert {course.id for course in get_courses(user, filter_=filter_)} == expected_courses, u'testing get_courses with filter_={}'.format(filter_)
...
class MongoCourseImageTestCase(ModuleStoreTestCase):
    ...
    def test_non_ascii_image_name(self):
        ...
        assert course_image_url(course) == '/c4x/{org}/{course}/asset/before___after.jpg'.format(org=course.location.org, course=course.location.course)

    def test_spaces_in_image_name(self):
        # Verify that image names with spaces in them are cleaned
        course = CourseFactory.create(course_image=u'before after.jpg')
        assert course_image_url(course) == '/c4x/{org}/{course}/asset/before_after.jpg'.format(org=course.location.org, course=course.location.course)
