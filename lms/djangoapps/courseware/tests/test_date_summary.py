...
class CourseDateSummaryTest(SharedModuleStoreTestCase):
    ...
    def test_course_end_date_after_course(self):
        ...
        CourseEnrollmentFactory(course_id=course.id, user=user, mode=CourseMode.VERIFIED)
        block = CourseEndDate(course, user)
        assert block.description == 'This course is archived, which means you can review course content but it is no longer active.'
        assert block.title == 'Course End'
    ...
    def test_verification_deadline_date_upcoming(self):
        with freeze_time('2015-01-02'):
            ...
            assert block.description == 'You must successfully complete verification before this date to qualify for a Verified Certificate.'
...
class TestScheduleOverrides(SharedModuleStoreTestCase):
    ...
    def _check_text(self, upgrade_date_summary):
        ...
        assert upgrade_date_summary.description == "Don't miss the opportunity to highlight your new knowledge and skills by earning a verified certificate."
