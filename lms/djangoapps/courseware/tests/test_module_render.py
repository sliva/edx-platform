...
class ModuleRenderTestCase(SharedModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_rebind_different_users(self, block_category):
        ...
        assert descriptor._unwrapped_field_data is original_field_data
        # lint-amnesty, pylint: disable=no-member
        assert descriptor._unwrapped_field_data is not descriptor._field_data
        # lint-amnesty, pylint: disable=no-member
        ...
        # pylint: disable=protected-access
        assert isinstance(descriptor._field_data, LmsFieldData)
        # lint-amnesty, pylint: disable=no-member

        # the LmsFieldData should now wrap OverrideFieldData
        assert isinstance(descriptor._field_data._authored_data._source, OverrideFieldData)

        # the OverrideFieldData should point to the date FieldData
        assert isinstance(descriptor._field_data._authored_data._source.fallback, DateLookupFieldData)
        assert descriptor._field_data._authored_data._source.fallback._defaults is descriptor._unwrapped_field_data
...
class TestHandleXBlockCallback(SharedModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_too_many_files(self):
        ...
        request.user = self.mock_user
        assert render.handle_xblock_callback(request, text_type(self.course_key), quote_slashes(text_type(self.location)), 'dummy_handler').content.decode('utf-8') == json.dumps({'success': (u'Submission aborted! Maximum %d files may be submitted at once' % settings.MAX_FILEUPLOADS_PER_INPUT)}, indent=2)
    ...
    def test_too_large_file(self):
        ...
        request.user = self.mock_user
        assert render.handle_xblock_callback(request, text_type(self.course_key), quote_slashes(text_type(self.location)), 'dummy_handler').content.decode('utf-8') == json.dumps({'success': (u'Submission aborted! Your file "%s" is too large (max size: %d MB)' % (inputfile.name, (settings.STUDENT_FILEUPLOAD_MAX_SIZE / (1000 ** 2))))}, indent=2)
...
class TestHtmlModifiers(ModuleStoreTestCase):
    ...
    def test_static_link_rewrite(self):
        ...
        assert '/c4x/{org}/{course}/asset/foo_content'.format(org=self.course.location.org, course=self.course.location.course) in result_fragment.content
    ...
    def test_static_badlink_rewrite(self):
        ...
        assert '/c4x/{org}/{course}/asset/file.jpg'.format(org=self.course.location.org, course=self.course.location.course) in result_fragment.content
...
class TestStaffDebugInfo(SharedModuleStoreTestCase):
    ...
    def test_staff_debug_info_score_for_invalid_dropdown(self):
        ...
        assert expected_score_override_html.format(block_id=problem_descriptor.location.block_id) in html_fragment.content
...
class TestAnonymousStudentId(SharedModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_per_student_anonymized_id(self, descriptor_class):
        for course_id in ('MITx/6.00x/2012_Fall', 'MITx/6.00x/2013_Spring'):
            assert 'de619ab51c7f4e9c7216b4644c24f3b5' == self._get_anonymous_id(CourseKey.from_string(course_id), descriptor_class)

    @ddt.data(*PER_COURSE_ANONYMIZED_DESCRIPTORS)
    def test_per_course_anonymized_id(self, descriptor_class):
        assert '0c706d119cad686d28067412b9178454' == self._get_anonymous_id(CourseKey.from_string('MITx/6.00x/2012_Fall'), descriptor_class)

        assert 'e9969c28c12c8efa6e987d6dbeedeb0b' == self._get_anonymous_id(CourseKey.from_string('MITx/6.00x/2013_Spring'), descriptor_class)
...
class TestRebindModule(TestSubmittingProblems):
    ...
    def test_rebind_noauth_module_to_user_anonymous(self):
        ...
        assert module._xmodule.scope_ids.user_id == user2.id
        # pylint: disable=protected-access
