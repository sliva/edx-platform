...
class ValidateTabsTestCase(TabListTestCase):
    ...
    def test_invalid_tab_type(self):
        ...
        """
        tab_list = xmodule_tabs.CourseTabList()
        assert len(tab_list.from_json([{'type': CoursewareTab.type}, {'type': CourseInfoTab.type, 'name': 'fake_name'}, {'type': 'no_such_type'}])) == 2
...
class CourseTabListTestCase(TabListTestCase):
    ...
    def test_iterate_displayable(self):
        ...
        assert {'type': 'html_textbooks'} in list(xmodule_tabs.CourseTabList.iterate_displayable(self.course, inline_collections=False))

        # test not including empty collections
        self.course.html_textbooks = []
        assert {'type': 'html_textbooks'} not in list(xmodule_tabs.CourseTabList.iterate_displayable(self.course, inline_collections=False))
...
class DiscussionLinkTestCase(TabTestCase):
    ...
    def check_discussion(
        ...
        with patch('common.djangoapps.student.models.CourseEnrollment.is_enrolled') as check_is_enrolled:
            ...
            assert ((discussion_tab is not None) and self.is_tab_enabled(discussion_tab, self.course, user) and (discussion_tab.link_func(self.course, self._reverse(self.course)) == expected_discussion_link)) == expected_can_display_value
