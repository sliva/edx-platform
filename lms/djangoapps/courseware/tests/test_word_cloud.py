...
class TestWordCloud(BaseTestXmodule):
    ...
    def test_initial_state(self):
        ...
        assert ''.join(set([content['status'] for (_, content) in users_state.items()])) == 'success'
    ...
    def test_collective_users_submits(self):
        ...
        assert ''.join(set([content['status'] for (_, content) in users_state_after_post.items()])) == 'success'
        ...
        assert ''.join(set([content['status'] for (_, content) in users_state_after_post.items()])) == 'fail'
