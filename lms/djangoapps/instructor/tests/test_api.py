...
class TestEndpointHttpMethods(SharedModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_endpoints_reject_get(self, data):
        ...
        assert response.status_code == 405, u'Endpoint {} returned status code {} instead of a 405. It should not allow GET.'.format(data, response.status_code)
    ...
    def test_endpoints_accept_get(self, data):
        ...
        assert response.status_code != 405, u"Endpoint {} returned status code 405 where it shouldn't, since it should allow GET.".format(data)
...
class TestInstructorAPIBulkAccountCreationAndEnrollment(SharedModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_file_upload_type_not_csv(self):
        ...
        data = json.loads(response.content.decode('utf-8'))
        assert len(data['general_errors']) != 0
        assert data['general_errors'][0]['response'] == 'Make sure that the file you upload is in CSV format with no extraneous characters or rows.'
    ...
    def test_insufficient_data(self):
        ...
        assert data['general_errors'][0]['response'] == 'Data in row #1 must have exactly four columns: email, username, full name, and country'
...
class TestInstructorAPIEnrollment(SharedModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_enroll_with_email(self, protocol):
        ...
        for body in [text_body, html_body]:
            assert u'You have been enrolled in {course_name} at edx.org by a member of the course staff.'.format(course_name=self.course.display_name) in body

            assert 'This course will now appear on your edx.org dashboard.' in body
            assert '{proto}://{site}{course_path}'.format(proto=protocol, site=self.site_name, course_path=self.course_path) in body
    ...
    def test_unenroll_with_email(self):
        ...
        # Check the outbox
        assert len(mail.outbox) == 1
        assert mail.outbox[0].subject == u'You have been unenrolled from {display_name}'.format(display_name=self.course.display_name)
    ...
    def test_enroll_with_email_not_registered_with_shib(self, protocol, mock_uses_shib):
        ...
        # Check the outbox
        assert len(mail.outbox) == 1
        assert mail.outbox[0].subject == u'You have been invited to register for {display_name}'.format(display_name=self.course.display_name)
...
class TestInstructorAPIBulkBetaEnrollment(SharedModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_add_notenrolled_with_email(self, protocol):
        ...
        # Check the outbox
        assert len(mail.outbox) == 1
        assert mail.outbox[0].subject == u'You have been invited to a beta test for {display_name}'.format(display_name=self.course.display_name)
    ...
    def test_remove_with_email(self):
        ...
        # Check the outbox
        assert len(mail.outbox) == 1
        assert mail.outbox[0].subject == u'You have been removed from a beta test for {display_name}'.format(display_name=self.course.display_name)
...
class TestInstructorAPILevelsDataDump(SharedModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_get_anon_ids(self):
        ...
        assert body.startswith('"User ID","Anonymized User ID","Course Specific Anonymized User ID"\n"{user_id}","41","42"\n'.format(user_id=self.students[0].id))
...
class TestInstructorAPIRegradeTask(SharedModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_reset_student_attempts_delete(self, _mock_signal):
        ...
        assert StudentModule.objects.filter(student=self.module_to_reset.student, course_id=self.module_to_reset.course_id).count() == 0
...
class TestInstructorSendEmail(SiteMixin, SharedModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_send_email_with_site_template_and_from_addr(self):
        ...
        response = self.client.post(url, self.full_test_message)
        assert response.status_code == 200
        assert 1 == CourseEmail.objects.filter(course_id=self.course.id, sender=self.instructor, subject=self.full_test_message['subject'], html_message=self.full_test_message['message'], template_name=site_template, from_addr=site_email).count()
    ...
    def test_send_email_with_org_template_and_from_addr(self):
        ...
        response = self.client.post(url, self.full_test_message)
        assert response.status_code == 200
        assert 1 == CourseEmail.objects.filter(course_id=self.course.id, sender=self.instructor, subject=self.full_test_message['subject'], html_message=self.full_test_message['message'], template_name=org_template, from_addr=org_email).count()
...
class TestDueDateExtensions(SharedModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_change_due_date(self):
        ...
        assert datetime.datetime(2013, 12, 30, 0, 0, tzinfo=UTC) == get_extended_due(self.course, self.week1, self.user1)
    ...
    def test_show_unit_extensions(self):
        ...
        assert response.status_code == 200, response.content
        assert json.loads(response.content.decode('utf-8')) == {u'data': [{u'Extended Due Date': u'2013-12-30 00:00', u'Full Name': self.user1.profile.name, u'Username': self.user1.username}], u'header': [u'Username', u'Full Name', u'Extended Due Date'], u'title': (u'Users with due date extensions for %s' % self.week1.display_name)}
    ...
    def test_show_student_extensions(self):
        ...
        assert response.status_code == 200, response.content
        assert json.loads(response.content.decode('utf-8')) == {u'data': [{u'Extended Due Date': u'2013-12-30 00:00', u'Unit': self.week1.display_name}], u'header': [u'Unit', u'Extended Due Date'], u'title': (u'Due date extensions for %s (%s)' % (self.user1.profile.name, self.user1.username))}
...
class TestCourseIssuedCertificatesData(SharedModuleStoreTestCase):
    ...
    def test_certificates_features_csv(self):
        ...
        assert response.content.strip().decode('utf-8') == (((('"CourseID","Certificate Type","Total Certificates Issued","Date Report Run"\r\n"' + str(self.course.id)) + '","honor","3","') + current_date) + '"')
