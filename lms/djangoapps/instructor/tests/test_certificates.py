...
class CertificatesInstructorApiTest(SharedModuleStoreTestCase):
    ...
    def test_certificate_regeneration_success(self):
        ...
        assert res_json['message'] == u'Certificate regeneration task has been started. You can view the status of the generation task in the "Pending Tasks" section.'
    ...
    def test_certificate_regeneration_error(self):
        ...
        assert res_json['message'] == u'Please select one or more certificate statuses that require certificate regeneration.'
...
class CertificateExceptionViewInstructorApiTest(SharedModuleStoreTestCase):
    ...
    def test_certificate_exception_invalid_username_error(self):
        ...
        # Assert Error Message
        assert res_json['message'] == u'{user} does not exist in the LMS. Please check your spelling and retry.'.format(user=invalid_user)
    ...
    def test_certificate_exception_missing_username_and_email_error(self):
        ...
        # Assert Error Message
        assert res_json['message'] == u'Student username/email field is required and can not be empty. Kindly fill in username/email and then press "Add to Exception List" button.'
    ...
    def test_certificate_exception_duplicate_user_error(self):
        ...
        # Assert Error Message
        assert res_json['message'] == u'Student (username/email={user_name}) already in certificate exception list.'.format(user_name=user)
    ...
    def test_certificate_exception_user_not_enrolled_error(self):
        ...
        # Assert Error Message
        assert res_json['message'] == u'{user} is not enrolled in this course. Please check your spelling and retry.'.format(user=self.certificate_exception['user_name'])
    ...
    def test_remove_certificate_exception_invalid_request_error(self):
        ...
        assert not res_json['success']
        # Assert Error Message
        assert res_json['message'] == u'The record is not in the correct format. Please add a valid username or email address.'
    ...
    def test_remove_certificate_exception_non_existing_error(self):
        ...
        assert not res_json['success']
        # Assert Error Message
        assert res_json['message'] == u'Certificate exception (user={user}) does not exist in certificate white list. Please refresh the page and try again.'.format(user=self.certificate_exception['user_name'])
...
class TestCertificatesInstructorApiBulkWhiteListExceptions(SharedModuleStoreTestCase):
    ...
    def test_file_upload_type_not_csv(self):
        ...
        assert len(data['general_errors']) != 0
        assert data['general_errors'][0] == 'Make sure that the file you upload is in CSV format with no extraneous characters or rows.'
...
class CertificateInvalidationViewTests(SharedModuleStoreTestCase):
    ...
    def test_missing_username_and_email_error(self):
        ...
        # Assert Error Message
        assert res_json['message'] == u'Student username/email field is required and can not be empty. Kindly fill in username/email and then press "Invalidate Certificate" button.'
    ...
    def test_user_not_enrolled_error(self):
        ...
        # Assert Error Message
        assert res_json['message'] == u'{user} is not enrolled in this course. Please check your spelling and retry.'.format(user=self.not_enrolled_student.username)
    ...
    def test_no_generated_certificate_error(self):
        ...
        # Assert Error Message
        assert res_json['message'] == u'The student {student} does not have certificate for the course {course}. Kindly verify student username/email and the selected course are correct and try again.'.format(student=self.enrolled_user_2.username, course=self.course.number)
    ...
    def test_certificate_already_invalid_error(self):
        ...
        # Assert Error Message
        assert res_json['message'] == u'Certificate for student {user} is already invalid, kindly verify that certificate was generated for this student and then proceed.'.format(user=self.enrolled_user_1.username)
    ...
    def test_duplicate_certificate_invalidation_error(self):
        ...
        # Assert Error Message
        assert res_json['message'] == u'Certificate of {user} has already been invalidated. Please check your spelling and retry.'.format(user=self.enrolled_user_1.username)
