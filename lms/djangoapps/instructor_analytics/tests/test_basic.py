...
class TestAnalyticsBasic(ModuleStoreTestCase):
    ...
    def test_enrolled_students_meta_features_keys(self):
        ...
        for userreport in userreports:
            self.assertEqual(set(userreport.keys()), set(query_features))
            self.assertIn(userreport['meta.position'], [f"edX expert {user.id}" for user in self.users])
            self.assertIn(userreport['meta.company'], [f"Open edX Inc {user.id}" for user in self.users])
