...
class TeamMembershipImportManagerTests(TeamMembershipEventTestMixin, SharedModuleStoreTestCase):
    ...
    def test_create_new_mixed_enrollment_team_fails(self):
        ...
        assert not result
        self.assert_no_events_were_emitted()
        assert self.import_manager.validation_errors[0] == 'Team new_exciting_team cannot have Master’s track users mixed with users in other tracks.'
        assert CourseTeam.objects.all().count() == 0
    ...
    def test_add_incompatible_mode_to_existing_unprotected_team_fails(self):
        ...
        assert not result
        self.assert_no_events_were_emitted()
        assert self.import_manager.validation_errors[0] == 'Team unprotected_team cannot have Master’s track users mixed with users in other tracks.'
    ...
    def test_add_incompatible_mode_to_existing_protected_team_fails(self):
        ...
        assert not result
        self.assert_no_events_were_emitted()
        assert self.import_manager.validation_errors[0] == 'Team protected_team cannot have Master’s track users mixed with users in other tracks.'
