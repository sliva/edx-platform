...
class TestCreateTeamAPI(EventTestMixin, TeamAPITestCase):
    ...
    def test_student_in_teamset(self):
        ...
        assert 'You are already in a team in this teamset.' == json.loads(response.content.decode('utf-8'))['user_message']
    ...
    def test_student_create_team_instructor_managed_topic(self, *args):  # pylint: disable=unused-argument
        ...
        assert "You can't create a team in an instructor managed topic." == json.loads(response.content.decode('utf-8'))['user_message']
    ...
    def test_full_student_creator(self):
        ...
        assert team == {'name': 'Fully specified team', 'language': 'fr', 'country': 'CA', 'topic_id': 'topic_1', 'course_id': str(self.test_course_1.id), 'description': 'Another fantastic team', 'organization_protected': False}
...
class TestBulkMembershipManagement(TeamAPITestCase):
    ...
    def test_deletion_via_upload_csv(self):
        ...
        assert not CourseTeamMembership.objects.filter(user_id=self.users[username].id, team__topic_id=topic_0_id).exists()
    ...
    def test_reassignment_via_upload_csv(self):
        ...
        assert CourseTeamMembership.objects.filter(user_id=self.users[username].id, team__topic_id=topic_0_id, team__name=windpower_team_name).exists()
        ...
        assert not CourseTeamMembership.objects.filter(user_id=self.users[username].id, team__topic_id=topic_0_id, team__name=windpower_team_name).exists()
        assert CourseTeamMembership.objects.filter(user_id=self.users[username].id, team__topic_id=topic_0_id, team__name=nuclear_team_name).exists()
    ...
    def test_upload_file_not_changed_csv(self):
        ...
        assert len(CourseTeamMembership.objects.filter(user_id=self.users[username].id, team__name=nuclear_team_name)) == 1
        assert CourseTeamMembership.objects.filter(user_id=self.users[username].id, team__name=nuclear_team_name).exists()
