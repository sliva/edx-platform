...
class TestPayAndVerifyView(UrlResetMixin, ModuleStoreTestCase, XssTestMixin, TestVerificationBase):
    ...
    def _get_page_data(self, response):
        ...
        assert pay_and_verify_div is not None, "Could not load pay and verify flow data.  Maybe this isn't the pay and verify page?"
...
class TestCheckoutWithEcommerceService(ModuleStoreTestCase):
    ...
    def test_create_basket(self):
        ...
        # Check the api call
        assert json.loads(httpretty.last_request().body.decode('utf-8')) == {'products': [{'sku': 'test-sku'}], 'checkout': True, 'payment_processor_name': 'test-processor'}
        # Check the response
...
class TestSubmitPhotosForVerification(MockS3BotoMixin, TestVerificationBase):
    ...
    def test_no_photo_id_and_no_initial_verification(self):
        ...
        assert response.content.decode('utf-8') == 'Photo ID image is required if the user does not have an initial verification attempt.'
...
class TestPhotoURLView(TestVerificationBase):
    ...
    def test_photo_url_view_returns_data(self):
        ...
        assert response.data['PhotoID'] == 'https://{bucket}/photo_id/{receipt_id}'.format(bucket=settings.AWS_S3_CUSTOM_DOMAIN, receipt_id=self.receipt_id)
        assert response.data['UserPhoto'] == 'https://{bucket}/face/{receipt_id}'.format(bucket=settings.AWS_S3_CUSTOM_DOMAIN, receipt_id=self.receipt_id)
