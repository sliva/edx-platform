...
class BookmarksListViewTests(BookmarksViewsTestsBase):
    ...
    def test_post_bookmark_for_non_existing_block(self):
        ...
        assert response.data['user_message'] == u'An error has occurred. Please try again.'
        assert response.data['developer_message'] == u'Block with usage_id: i4x://arbi/100/html/340ef1771a094090ad260ec940d04a21 not found.'
    ...
    def test_post_bookmark_when_max_bookmarks_already_exist(self):
        ...
        assert response.data['user_message'] == u'You can create up to {0} bookmarks. You must remove some bookmarks before you can add new ones.'.format(max_bookmarks)
        assert response.data['developer_message'] == u'You can create up to {0} bookmarks. You must remove some bookmarks before you can add new ones.'.format(max_bookmarks)
...
class BookmarksDetailViewTests(BookmarksViewsTestsBase):
    ...
    def test_get_bookmark_that_does_not_exist(self):
        ...
        assert response.data['user_message'] == 'Bookmark with usage_id: i4x://arbi/100/html/340ef1771a0940 does not exist.'
        assert response.data['developer_message'] == 'Bookmark with usage_id: i4x://arbi/100/html/340ef1771a0940 does not exist.'
    ...
    def test_delete_bookmark_that_does_not_exist(self):
        ...
        assert response.data['user_message'] == u'Bookmark with usage_id: i4x://arbi/100/html/340ef1771a0940 does not exist.'
        assert response.data['developer_message'] == 'Bookmark with usage_id: i4x://arbi/100/html/340ef1771a0940 does not exist.'
