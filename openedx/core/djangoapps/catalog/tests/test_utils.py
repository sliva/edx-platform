...
class TestGetPrograms(CacheIsolationTestCase):
    ...
    def test_get_many(self, mock_warning, mock_info):
        ...
        assert set((program['uuid'] for program in actual_programs)) == set((program['uuid'] for program in partial_programs.values()))
        ...
        assert set((program['uuid'] for program in actual_programs)) == set((program['uuid'] for program in all_programs.values()))
...
class TestGetPathways(CacheIsolationTestCase):
    ...
    def test_get_many(self, mock_warning, mock_info):
        ...
        # messages should be logged for the missing one.
        assert set((pathway['id'] for pathway in actual_pathways)) == set((pathway['id'] for pathway in partial_pathways.values()))
        ...
        assert set((pathway['id'] for pathway in actual_pathways)) == set((pathway['id'] for pathway in all_pathways.values()))
        assert not mock_warning.called
...
class TestGetCourseRuns(CatalogIntegrationMixin, CacheIsolationTestCase):
    ...
    def assert_contract(self, call_args):
        ...
        assert kwargs['api']._store['base_url'] == self.catalog_integration.get_internal_api_url()
