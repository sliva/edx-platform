    ...
    def test_enqueue(self, enqueue_task, force_update, routing_key):
        ...
        with patch(
            ...
            with patch(
                ...
                assert mock_tasks.update_course_in_cache_v2.apply_async.call_count == (self.num_courses if (enqueue_task and force_update) else 0)
                assert mock_tasks.get_course_in_cache_v2.apply_async.call_count == (self.num_courses if (enqueue_task and (not force_update)) else 0)

                assert mock_api.update_course_in_cache.call_count == (self.num_courses if ((not enqueue_task) and force_update) else 0)
                assert mock_api.get_course_in_cache.call_count == (self.num_courses if ((not enqueue_task) and (not force_update)) else 0)
