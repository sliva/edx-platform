...
class TestBlockStructureTransformers(ChildrenMapTestMixin, TestCase):
    ...
    def test_add_registered(self):
        ...
        assert self.registered_transformers[0] in self.transformers._transformers['no_filter']
        assert self.registered_transformers[1] in self.transformers._transformers['supports_filter']

    def test_add_unregistered(self):
        with pytest.raises(TransformerException):
            self.transformers += [self.UnregisteredTransformer()]

        assert self.transformers._transformers['no_filter'] == []
        # pylint: disable=protected-access
        assert self.transformers._transformers['supports_filter'] == []
        # pylint: disable=protected-access
