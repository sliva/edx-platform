...
class CourseOverviewTestCase(CatalogIntegrationMixin, ModuleStoreTestCase, CacheIsolationTestCase):
    ...
    def test_get_all_courses_by_org(self):
        ...
        assert {c.id for c in CourseOverview.get_all_courses()} == {c.id for c in ((org_courses[0] + org_courses[1]) + org_courses[2])}

        assert {c.id for c in CourseOverview.get_all_courses(orgs=['test_org_1', 'test_org_2'])} == {c.id for c in (org_courses[1] + org_courses[2])}
    ...
    def test_get_all_courses_by_mobile_available(self):
        ...
        for filter_, expected_courses in test_cases:
            assert {course_overview.id for course_overview in CourseOverview.get_all_courses(filter_=filter_)} == expected_courses, u'testing CourseOverview.get_all_courses with filter_={}'.format(filter_)
