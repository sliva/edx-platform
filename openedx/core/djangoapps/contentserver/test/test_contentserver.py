...
class ContentStoreToyCourseTest(SharedModuleStoreTestCase):
    ...
    def test_range_request_full_file(self):
        ...
        assert resp['Content-Range'] == u'bytes {first}-{last}/{length}'.format(first=0, last=(self.length_unlocked - 1), length=self.length_unlocked)
    ...
    def test_range_request_partial_file(self):
        ...
        assert resp.status_code == 206
        # HTTP_206_PARTIAL_CONTENT
        assert resp['Content-Range'] == u'bytes {first}-{last}/{length}'.format(first=first_byte, last=last_byte, length=self.length_unlocked)
