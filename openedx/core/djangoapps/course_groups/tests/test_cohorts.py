...
class TestCohorts(ModuleStoreTestCase):
    ...
    def test_cannot_set_assignment_type(self):
        ...
        assert exception_msg == str(context_manager.exception)
    ...
    def test_get_cohort(self):
        ...
        assert cohorts.get_cohort(other_user, course.id).id == cohorts.get_cohort_by_name(course.id, cohorts.DEFAULT_COHORT_NAME).id, 'other_user should be assigned to the default cohort'
    ...
    def test_cohorting_with_migrations_done(self):
        ...
        assert cohorts.get_cohort(user1, course.id).name == 'AutoGroup', 'user1 should still be in originally placed cohort'
    ...
    def test_cohorting_with_no_auto_cohorts(self):
        ...
        assert cohorts.get_cohort(user1, course.id).id == cohorts.get_cohort_by_name(course.id, cohorts.DEFAULT_COHORT_NAME).id, 'No groups->default cohort for user1'
        ...
        assert cohorts.get_cohort(user1, course.id).name == cohorts.get_cohort_by_name(course.id, cohorts.DEFAULT_COHORT_NAME).name, 'user1 should still be in the default cohort'

        assert cohorts.get_cohort(user2, course.id).id == cohorts.get_cohort_by_name(course.id, cohorts.DEFAULT_COHORT_NAME).id, 'No groups->default cohort for user2'
    ...
    def test_get_cohort_by_name(self):
        ...
        pytest.raises(CourseUserGroup.DoesNotExist, (lambda : cohorts.get_cohort_by_name(course.id, 'CohortDoesNotExist')))

        cohort = CohortFactory(course_id=course.id, name="MyCohort")

        assert cohorts.get_cohort_by_name(course.id, 'MyCohort') == cohort

        pytest.raises(CourseUserGroup.DoesNotExist, (lambda : cohorts.get_cohort_by_name(CourseLocator('course', 'does_not', 'exist'), cohort)))
    ...
    def test_set_cohorted_with_invalid_data_type(self):
        ...
        assert 'Cohorted must be a boolean' == text_type(value_error.exception)
...
class TestCohortsAndPartitionGroups(ModuleStoreTestCase):
    ...
    def test_multiple_cohorts(self):
        ...
        assert cohorts.get_group_info_for_cohort(self.first_cohort) == (self.group1_id, self.partition_id)
        assert cohorts.get_group_info_for_cohort(self.second_cohort) == cohorts.get_group_info_for_cohort(self.first_cohort)
