...
class CohortHandlerTestCase(CohortViewsTestCase):
    ...
    def verify_lists_expected_cohorts(self, expected_cohorts, response_dict=None, user=None):
        ...
        assert response_dict.get('cohorts') == [{'name': cohort.name, 'id': cohort.id, 'user_count': cohort.user_count, 'assignment_type': cohort.assignment_type, 'user_partition_id': None, 'group_id': None} for cohort in expected_cohorts]
    ...
    def test_get_single_cohort(self):
        ...
        assert response_dict == {'name': self.cohort2.name, 'id': self.cohort2.id, 'user_count': 2, 'assignment_type': CourseCohort.MANUAL, 'user_partition_id': None, 'group_id': None}
    ...
    def verify_contains_added_cohort(
        ...
        assert response_dict == {'name': cohort_name, 'id': created_cohort.id, 'user_count': 0, 'assignment_type': assignment_type, 'user_partition_id': expected_user_partition_id, 'group_id': expected_group_id}
...
class AddUsersToCohortTestCase(CohortViewsTestCase):
    ...
    def verify_added_users_to_cohort(self, response_dict, cohort, course, expected_added, expected_changed,
        ...
        """
        assert response_dict.get('success')
        assert response_dict.get('added') == [{'username': user.username, 'email': user.email} for user in expected_added]
        assert response_dict.get('changed') == [{'username': user.username, 'email': user.email, 'previous_cohort': previous_cohort} for (user, previous_cohort) in expected_changed]
        ...
        for user in expected_added + [user for (user, _) in expected_changed + expected_present]:
            assert CourseUserGroup.objects.get(course_id=course.id, group_type=CourseUserGroup.COHORT, users__id=user.id) == cohort
