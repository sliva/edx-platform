...
class CreditProviderIntegrationApiTests(CreditApiTestBase):
    ...
    def test_get_credit_providers(self):
        ...
        assert result == [{'id': self.PROVIDER_ID, 'display_name': self.PROVIDER_NAME, 'url': self.PROVIDER_URL, 'status_url': self.PROVIDER_STATUS_URL, 'description': self.PROVIDER_DESCRIPTION, 'enable_integration': self.ENABLE_INTEGRATION, 'fulfillment_instructions': self.FULFILLMENT_INSTRUCTIONS, 'thumbnail_url': self.THUMBNAIL_URL}]
