...
class TestSafeCookieData(TestSafeSessionsLogMixin, TestCase):
    ...
    def test_digest_success(self):
        ...
        assert self.safe_cookie_data._compute_digest(self.user_id) == self.safe_cookie_data._compute_digest(self.user_id)

    @ddt.data('another_user', 0, None)
    def test_digest_incorrect_user(self, incorrect_user):
        assert self.safe_cookie_data._compute_digest(self.user_id) != self.safe_cookie_data._compute_digest(incorrect_user)
