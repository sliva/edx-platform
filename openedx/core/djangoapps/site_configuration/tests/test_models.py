...
class SiteConfigurationTests(TestCase):
    ...
    def test_get_value(self):
        ...
        assert site_configuration.get_value('course_about_show_social_links') == self.test_config1['course_about_show_social_links']
    ...
    def test_get_value_for_org(self):
        ...
        # Make sure entry is saved and retrieved correctly
        assert SiteConfiguration.get_value_for_org(self.test_config1['course_org_filter'], 'university') == self.test_config1['university']
        assert SiteConfiguration.get_value_for_org(self.test_config1['course_org_filter'], 'platform_name') == self.test_config1['platform_name']
        assert SiteConfiguration.get_value_for_org(self.test_config1['course_org_filter'], 'SITE_NAME') == self.test_config1['SITE_NAME']
        assert SiteConfiguration.get_value_for_org(self.test_config1['course_org_filter'], 'css_overrides_file') == self.test_config1['css_overrides_file']
        assert SiteConfiguration.get_value_for_org(self.test_config1['course_org_filter'], 'ENABLE_MKTG_SITE') == self.test_config1['ENABLE_MKTG_SITE']

        # Make sure entry is saved and retrieved correctly
        assert SiteConfiguration.get_value_for_org(self.test_config2['course_org_filter'], 'university') == self.test_config2['university']
        assert SiteConfiguration.get_value_for_org(self.test_config2['course_org_filter'], 'platform_name') == self.test_config2['platform_name']
        assert SiteConfiguration.get_value_for_org(self.test_config2['course_org_filter'], 'SITE_NAME') == self.test_config2['SITE_NAME']
        assert SiteConfiguration.get_value_for_org(self.test_config2['course_org_filter'], 'css_overrides_file') == self.test_config2['css_overrides_file']
        assert SiteConfiguration.get_value_for_org(self.test_config2['course_org_filter'], 'ENABLE_MKTG_SITE') == self.test_config2['ENABLE_MKTG_SITE']

        # Test that the default value is returned if the value for the given key is not found in the configuration
        assert SiteConfiguration.get_value_for_org(self.test_config1['course_org_filter'], 'non-existent', 'dummy-default-value') == 'dummy-default-value'

        # Test that the default value is returned if the value for the given key is not found in the configuration
        assert SiteConfiguration.get_value_for_org(self.test_config2['course_org_filter'], 'non-existent', 'dummy-default-value') == 'dummy-default-value'

        # Test that the default value is returned if org is not found in the configuration
        assert SiteConfiguration.get_value_for_org('non-existent-org', 'platform_name', 'dummy-default-value') == 'dummy-default-value'
