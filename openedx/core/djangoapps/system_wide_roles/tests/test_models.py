...
class SystemWideRoleAssignmentTests(TestCase):
    ...
    def test_str(self):
        ...
        assert str(role_assignment) == '<SystemWideRoleAssignment for User {user} assigned to role {role}>'.format(user=self.user.id, role=self.role.name)

    def test_repr(self):
        role_assignment = SystemWideRoleAssignment.objects.create(role=self.role, user=self.user)
        assert repr(role_assignment) == '<SystemWideRoleAssignment for User {user} assigned to role {role}>'.format(user=self.user.id, role=self.role.name)
