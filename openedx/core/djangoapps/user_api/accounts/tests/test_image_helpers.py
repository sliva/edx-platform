...
class ProfileImageUrlTestCase(TestCase):
    ...
    def verify_url(self, actual_url, expected_name, expected_pixels, expected_version):
        ...
        """
        assert actual_url == 'http://example-storage.com/profile-images/{name}_{size}.jpg?v={version}'.format(name=expected_name, size=expected_pixels, version=expected_version)
