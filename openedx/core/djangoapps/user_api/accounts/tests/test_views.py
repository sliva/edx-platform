...
class UserAPITestCase(APITestCase):
    ...
    def _verify_profile_image_data(self, data, has_profile_image):
        ...
        assert data['profile_image'] == {'has_image': has_profile_image, 'image_url_full': template.format(size=50), 'image_url_small': template.format(size=10)}
...
class TestAccountsAPI(CacheIsolationTestCase, UserAPITestCase):
    ...
    def test_patch_account(self, field, value, fails_validation_value=None, developer_validation_message=None):
        ...
        if fails_validation_value:
            ...
            assert u"Value '{value}' is not valid for field '{field}': {messages}".format(value=fails_validation_value, field=field, messages=[developer_validation_message]) == error_response.data['field_errors'][field]['developer_message']
    ...
    def test_patch_account_noneditable(self):
        ...
        def verify_error_response(field_name, data):
            ...
            assert u"The '{0}' field cannot be edited.".format(field_name) == data['field_errors'][field_name]['user_message']
    ...
    def test_patch_invalid_email(self, bad_email):
        ...
        assert "Error thrown from validate_new_email: 'Valid e-mail address required.'" == field_errors['email']['developer_message']
    ...
    def test_patch_invalid_language_proficiencies(self, patch_value, expected_error_message):
        ...
        assert response.data['field_errors']['language_proficiencies']['developer_message'] == u"Value '{patch_value}' is not valid for field 'language_proficiencies': {error_message}".format(patch_value=patch_value, error_message=expected_error_message)
    ...
    def test_convert_relative_profile_url(self):
        ...
        self.client.login(username=self.user.username, password=TEST_PASSWORD)
        response = self.send_get(self.client)
        assert response.data['profile_image'] == {'has_image': False, 'image_url_full': 'http://testserver/static/default_50.png', 'image_url_small': 'http://testserver/static/default_10.png'}
