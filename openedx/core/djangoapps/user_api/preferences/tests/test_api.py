...
class TestPreferenceAPI(CacheIsolationTestCase):
    ...
    def test_get_user_preference(self):
        ...
        assert get_user_preference(self.staff_user, self.test_preference_key, username=self.user.username) == self.test_preference_value
    ...
    def test_set_user_preference_errors(self, user_preference_save):
        ...
        assert errors[too_long_key] == {'developer_message': get_expected_validation_developer_message(too_long_key, 'new_value'), 'user_message': get_expected_key_error_user_message(too_long_key, 'new_value')}
        ...
        for empty_value in (None, "", "   "):
            ...
            errors = context_manager.exception.preference_errors
            assert len(list(errors.keys())) == 1
            assert errors[self.test_preference_key] == {'developer_message': get_empty_preference_message(self.test_preference_key), 'user_message': get_empty_preference_message(self.test_preference_key)}
        ...
        assert context_manager.exception.developer_message == u"Save failed for user preference 'new_key_ȻħȺɍłɇs' with value 'new_value_ȻħȺɍłɇs': "
        assert context_manager.exception.user_message == u"Save failed for user preference 'new_key_ȻħȺɍłɇs' with value 'new_value_ȻħȺɍłɇs'."
    ...
    def test_update_user_preferences_errors(self, user_preference_save, user_preference_delete):
        ...
        assert context_manager.exception.developer_message == u"Save failed for user preference 'test_key' with value 'new_value': "
        assert context_manager.exception.user_message == u"Save failed for user preference 'test_key' with value 'new_value'."

        user_preference_delete.side_effect = [Exception, None]
        with pytest.raises(PreferenceUpdateError) as context_manager:
            update_user_preferences(self.user, {self.test_preference_key: None})
        assert context_manager.exception.developer_message == u"Delete failed for user preference 'test_key': "
        assert context_manager.exception.user_message == u"Delete failed for user preference 'test_key'."
