...
class TestPreferencesAPI(UserAPITestCase):
    ...
    def test_update_preferences_bad_request(self):
        ...
        assert response.data == {'developer_message': u'No data provided for user preference update', 'user_message': u'No data provided for user preference update'}
...
class TestPreferencesDetailAPI(UserAPITestCase):
    ...
    def test_create_empty_preference(self, preference_value):
        ...
        assert response.data == {'developer_message': u"Preference 'new_key' cannot be set to an empty value.", 'user_message': u"Preference 'new_key' cannot be set to an empty value."}
    ...
    def test_create_preference_too_long_key(self):
        ...
        assert response.data == {'developer_message': get_expected_validation_developer_message(too_long_preference_key, new_value), 'user_message': get_expected_key_error_user_message(too_long_preference_key, new_value)}
    ...
    def test_update_preference_to_empty(self, preference_value):
        ...
        self.client.login(username=self.user.username, password=TEST_PASSWORD)
        response = self.send_put(self.client, preference_value, expected_status=400)
        assert response.data == {'developer_message': u"Preference 'test_key' cannot be set to an empty value.", 'user_message': u"Preference 'test_key' cannot be set to an empty value."}
        response = self.send_get(self.client)
        assert self.test_pref_value == response.data
