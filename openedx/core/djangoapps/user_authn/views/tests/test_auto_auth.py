...
class AutoAuthEnabledTestCase(AutoAuthTestCase, ModuleStoreTestCase):
    ...
    def test_set_roles(self, course_id, course_key):
        ...
        user_roles = user.roles.all()
        assert set(user_roles) == set([course_roles[FORUM_ROLE_STUDENT], course_roles[FORUM_ROLE_MODERATOR], course_roles[FORUM_ROLE_ADMINISTRATOR]])
    ...
    def test_course_access_roles(self):
        ...
        for role in expected_roles:
            assert CourseAccessRole.objects.filter(user__id=user_info['user_id'], course_id=course_key, org=course_key.org, role=role).exists()
