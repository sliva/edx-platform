...
class LearnerProfileViewTest(SiteMixin, UrlResetMixin, ModuleStoreTestCase):
    ...
    def test_context(self):
        ...
        assert context['data']['default_public_account_fields'] == settings.ACCOUNT_VISIBILITY_CONFIGURATION['public_fields']

        assert context['data']['accounts_api_url'] == reverse('accounts_api', kwargs={'username': self.user.username})

        assert context['data']['preferences_api_url'] == reverse('preferences_api', kwargs={'username': self.user.username})

        assert context['data']['profile_image_upload_url'] == reverse('profile_image_upload', kwargs={'username': self.user.username})

        assert context['data']['profile_image_remove_url'] == reverse('profile_image_remove', kwargs={'username': self.user.username})
